...
class Mine(BaseAction):
    ...
    def clickMinepage(self):
        self.find_ele(MineLoc.Mine)
        self.click_element(MineLoc.Mine,'我的模块')

    def getUserName(self):
        username=self.getElementText(MineLoc.Mine_userName)
        return username
    def clickUserInfo(self):
        self.click_ele(MineLoc.Mine_userInfoBtn)
    def userInfoTitle(self):
        try:
            title=self.getElementText(MineLoc.Mine_userInfo_title)
            return title
        except:
            return False